...
impl ValidatableExperiment for ExpStruct {
    fn experiment_id(&self) -> &str { "x8_deep_research" }
    fn name(&self) -> &str { "Deep Research Agent Accuracy — OpenSeeker-v2" }
    fn domain(&self) -> ResearchDomain { ResearchDomain::DeepResearch }
    fn lifecycle_stage(&self) -> LifecycleStage { LifecycleStage::PerTrainingCycle }
    fn nl_description(&self) -> &str { "Evaluate domain-trained OpenSeeker-v2 on AutoResearchBench, BrowseComp, HLE, and xbench" }
