...
impl ValidatableExperiment for ExpStruct {
    fn experiment_id(&self) -> &str { "x11_genui_compliance" }
    fn name(&self) -> &str { "Generative UI Compliance & Hallucination Rate" }
    fn domain(&self) -> ResearchDomain { ResearchDomain::GenerativeUI }
    fn lifecycle_stage(&self) -> LifecycleStage { LifecycleStage::PerRelease }
    fn nl_description(&self) -> &str { "Evaluate A2UI v0.9 spec compliance, WCAG 2.1 AA pass rate, and hallucination rate with vs. without UX Middleware" }
