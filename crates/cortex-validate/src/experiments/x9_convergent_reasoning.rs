...
impl ValidatableExperiment for ExpStruct {
    fn experiment_id(&self) -> &str { "x9_convergent_reasoning" }
    fn name(&self) -> &str { "Convergent Reasoning Factuality" }
    fn domain(&self) -> ResearchDomain { ResearchDomain::ConvergentReasoning }
    fn lifecycle_stage(&self) -> LifecycleStage { LifecycleStage::PerRelease }
    fn nl_description(&self) -> &str { "Evaluate 3-path convergent reasoning against single-model baselines on enterprise factuality dataset" }
