...
impl ValidatableExperiment for ExpStruct {
    fn experiment_id(&self) -> &str { "x10_wellness_correlation" }
    fn name(&self) -> &str { "Wellness Multimodal Correlation" }
    fn domain(&self) -> ResearchDomain { ResearchDomain::MultiModalWellness }
    fn lifecycle_stage(&self) -> LifecycleStage { LifecycleStage::Continuous }
    fn nl_description(&self) -> &str { "Evaluate Cortex Pulse composite score correlation with PHQ-9, GAD-7, and MBI clinical instruments" }
