...
pub fn router(gateway: Arc<SemanticGateway>, provenance: Arc<ProvenanceEngine>, firewall: Arc<SemanticFirewall>) -> Router {
    Router::new()
...
#[derive(Clone)]
struct AppState { gateway: Arc<SemanticGateway>, provenance: Arc<ProvenanceEngine>, firewall: Arc<SemanticFirewall> }

async fn health_check() -> &'static str { "ok" }
async fn kill() -> impl IntoResponse { KILL_SWITCH.store(true, Ordering::SeqCst); (StatusCode::OK, Json(serde_json::json!({"status":"killed"}))) }
async fn revive() -> impl IntoResponse { KILL_SWITCH.store(false, Ordering::SeqCst); (StatusCode::OK, Json(serde_json::json!({"status":"revived"}))) }
...
async fn handle_mcp(
    ...
    let plan = match state.gateway.route_intent(intent).await {
        Ok(p) => p,
        Err(e) => return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error":e.to_string()}))).into_response(),
    };
    let result = state.gateway.execute_plan(plan.clone()).await;
    let mut acc = state.provenance.accumulator.write().await;
    let capsule = acc.attach(uuid::Uuid::new_v4(), cortex_provenance::tracecaps::ActionKind::ToolCall, &[]);
    let mut ledger = state.provenance.ledger.write().await;
    ledger.append(serde_json::to_string(&capsule).unwrap_or_default());
    (StatusCode::OK, Json(serde_json::json!({"plan":plan,"result":result,"capsule_id":capsule.id.to_string()}))).into_response()
}
